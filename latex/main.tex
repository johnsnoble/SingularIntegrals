...
\end{equation}
As mentioned in\cite{logkernel}, we can take:
$$\int_A log||z-t||f(t)dt=Re\mathcal{L}_A \text{ and }\int_A\nabla log||z-t||f(t)dt=\mathcal{S}_A^*$$
The first one fairly intuitive given that we are interested in the distances which are given by the real component of the log function.
The second one we will derive later in \ref{bem} in more detail.
It is useful to be able to solve for the derivative of the kernel so that we can use it in the boundary element method\cite{bem} to compute a homogeneous version of the equation for some boundary conditions which we can add on to make the non-homogeneous version fit desired boundary conditions.
...
\end{equation}
Before moving to the more complex recursive relations in the case of the trapezium, it is useful to visit the Stieltjes transform recurrences on an intervel and a square as done in\cite{logkernel}.
...
\end{split}
\end{equation}
Extending this to work over a square using the recurrence over intervals:
\begin{equation}
\begin{split}
...
The motivation to do this will become clear later on when we attempt to use boundary element methods\cite{bem} to help solve a poisson PDE.
...
We will show why the solution to a cauchy transform across this interval is as follows in\cite{fastalgorithms}:
\begin{equation}
...
This can be done by computing for a Trapezium,
...
This would result in unstable results even though the recurrences are more simpler.
...
\begin{align}
    ...
    &+S_{kj-1}B_{jj-1}+S_{kj+1}B_{jj+1}+C\\
\end{align}
...
Since our recurrence relation has the "diagonal" elements we cannot use this form.
This prompts us to use a slightly different form where we have access to diagonal elements by shifting.
For example the matrix $A$ can currently only take values from the column of $S$ but needs to take from the neighbouring columns also.
Therefore this motivates us to try and shift the matrix $S$ before accessing it
$$(z-\alpha)S = AS + A_SP_- + A_+SP_+ + SB^T + Q_-SB_-^T + Q_+SB_+^T + C$$
Where $P_-,P_+,Q_-,Q_+$ are shift matricies which shift matricies which move the columns, rows respectively.
However, this not the correct form we may not be able to guarantee displacement structure.
...
From the findings in \eqref{branch_correct}, the interval intercepts the real axis in the postives $x^*$ for all $s^*$ which means that we never have an issue with branch cut.
...
Splitting the trapezium into lines for all $s$, we have from \eqref{branch_correct}, that we only have to correct for branch crossing depending on the value of $x^*$ which it would cross the real axis at.
...
\end{align}
Given \eqref{branch_correct}, we have that if $s>s^*$ we can say that the whole interval for that $s$ has crossed the branch cut and therefore, we compensate with the correction:$-2\pi i\mathbb{I}\{s>s^*\}$ for all $t$.
...
\subsection{Accuracy}
Accuracy is difficult to try and quantify with this algorithm since it except from the initial function approximation onto our function space, we have exact results.
In theory the results are exact however, we may have small errors due to rounding which can be further improved with the use of Double64 over Float64.
To eliminate as a factor how well we do function approximation, we will now test the potential of function which is from the function space we are working with to see how the quadrature result differs from the recursive results by plotting the difference on a $log_{10}$ scale:

\includegraphics[width=.5\textwidth]{potentialp5}
\includegraphics[width=.5\textwidth]{potentialp10}
This was compared to a quadrature result which has a relatively high tolerance to speed up computation which means we do expect to see small differences.
The value of the error changes only by a small amount from using p=5(left) to p=10(right), which further shows that the error is from the quadrature.
...
\end{align}
Once we have the value of $t$ at which we have the branch crossing, we can add on the correction in the form of:
$$\pm 2\pi\int_t^1\int_{-1}^1log(z-(\alpha + \beta)t-s)ds$$
...
We will be able to do this using the boundary element method \cite{bem}.
...
We have $O(np^2)$ and now it has started to increase in size.
\section{Conclusion}
We were able to successfully derive and implement recurrence relations which works over trapeziums and therefore triangles although much more complex.
This algorithm is also shown to have performed orders of magnitude faster than the traditional quadrature methods.
The added complexity interfered with the displacement property which we were hoping for.
Although, no rigorous proof has been done to show that a recurrence over trapeziums which has the displacement property does not exist it seems fairly convincing that it doesn't exist.
Future work could be to either be to find a form in which displacement property holds or a proof otherwise.
Another direction would be to try and generalise even further in the direction of new dimensions to see if we can find this property there.

\section{Appendix}
Github link to repo: https://github.com/johnsnoble/SingularIntegrals

BaseCases.jl: Basecases for stieltjes case

BEM.jl: Boundary element method to find homogeneous solutions

Common.jl: Useful function used throughout

Intervals.jl: Recurrences on simple intervals (not all were used)

LogTrap.jl: Functions that give L matrix for Log kernel on trapezium
use function $l_{kj}$ to get the whole square

MultivariateTesting.jl: A few test cases

TestUtil.jl: Testing utility

StieltjesTrap.jl: Stieltjes on trapezium
Use function s\_trap\_matrix! to get the S matrix.
