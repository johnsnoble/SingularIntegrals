...
If we want to translate the region of interest from $[-1,1]\times[-1,1]$ to $[x-1,x+1],[y-1,y+1]$, we need to change the region to $s+x+i(t+y)$.
This lets us rewrite the denominator as $(z-x-iy)-(s+it)$.
Therefore the same recursion rules but with a different value of $z$ gives us this.

\subsubsection{Scaling and Rotation}
Another type of translation we can make is to do with multipling the region of interest by some $\alpha\in\mathbb{C}$.
The real component of this value contributes to the scaling and the complex component the rotation.
We can therefore rewrite the scaled $s_{kj}(z)$ as $\frac{1}{\alpha}s_{kj}(\frac{z}{a})$
