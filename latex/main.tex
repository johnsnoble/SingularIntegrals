...
\subsubsection{Case 1: $k,j>1$}
\begin{align}
    zL_{kj} &= \int_{-1}^1zP_j(t)L_k(\tilde{z}_t)dt\\
    ...
    &+\alpha L_{kj}+(\beta+i)(j_-L_{kj-1}+j_+L_{kj+1})\\
    &=\alpha(k_-^LL_{k-1j}+k_+^LL_{k+1j})+
    \beta(k_-^Lj_-L_{k-1j-1}+k_-^Lj_+L_{k-1j+1}\\
    &+k_+^Lj_-L_{k+1j-1}+k_+^Lj_+L_{k+1j+1})+\alpha L_{kj}+(\beta+i)(j_-L_{kj-1}+j_+L_{kj+1})\\
