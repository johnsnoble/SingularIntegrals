...
$$log(z+i-(\alpha-\beta)(1+s)) -> log(\frac{\alpha-\beta}{i})+log(\frac{i(z+i)}{\alpha-\beta}-i-is)$$
...
However solving for when $Im(z)>0$, $Re(z)<2(\alpha-\beta)$ gives:
\begin{align}
    Re(z)-(\alpha-\beta)(1+s_1)&:=0\\
    \frac{Re(z)}{\alpha-\beta}-1&=s_1\\
    \int_{-1}^1f(s)log(z+i-(\alpha-\beta)(1+s))ds&=-2\pi i\int_s^1f(s)ds\\
