...
\end{align}
Where $C_1(t)$ is a correction term as a result of branch cuts from splitting up logs which we will characterise later
...
\begin{align}
    ...
    &+\int_{t_2}^{t_1}f(t)(log(z_3)+2\pi)dt+\int_{t_1}^1f(t)log(z_3)dt\\
    &=log(c_1)\int_{-1}^1f(t)dt+\int_{-1}^1f(t)log(z_3)dt+2\pi\int_{t_2}^{t_1}f(t)dt
\end{align}
Therefore we can write our earlier expression for the correction term as:
$$C_1(t)=2\pi \mathbb{I}_{\{t_2<t<t_1\}}$$
...
\begin{align}
    ...
    \implies \int_{-1}^1P_j(t)C_1(t)&=2\pi\int_{t_2}^{t_1}P_j(t)dt\\
    &=2\pi(C_{k+1}^{(-1/2)}(t_2)-C_{k+1}^{(-1/2)}(t_1))
\end{align}
