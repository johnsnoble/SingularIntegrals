...
$$z\notin \{z: im(z)\in [-1,1], re(z)\in [0,2\alpha+2\beta\cdot im(z)]\}$$
We can now consider two cases.
let:
\begin{align}
    z_1&=z-it\\
    z_2&=z-2\alpha-2\beta-it\\
    k&=2\alpha+2\beta\\
    &\implies (re(z_1)-k, im(z_1))=(re(z_2), im(z_2))
\end{align}
...
Next we address:
$$log(z-2\alpha-(2\beta+i)t)=log(\frac{2\beta+i}{i})+log(\frac{(z-2\alpha)i}{2\beta+i}-it))$$
